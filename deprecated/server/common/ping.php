...
<table id="mytable" class="table table-striped">
	...
	<tbody>
		...
		try {
			$sql = <<<TXT
SELECT
	places.name AS pname,
	eq3.grname AS grname,
	eq3.ip AS ip,
	eq3.nomename AS nomename
FROM places
	INNER JOIN (
		SELECT
			eq2.placesid AS placesid,
			group_nome.name AS grname,
			eq2.ip AS ip,
			eq2.nomename AS nomename
		FROM group_nome
			INNER JOIN (
				SELECT
					eq.placesid AS placesid,
				...
				FROM nome
					INNER JOIN (
						SELECT
							equipment.placesid AS placesid,
					...
					) AS eq ON eq.nomeid = nome.id
			) AS eq2 ON eq2.groupid = group_nome.id
	) AS eq3 ON places.id = eq3.placesid
TXT;
